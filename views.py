...
def pastepad(padid):
    ...
    if checkpad.check(padid):
        pasteres =  checkpad.check(padid)
...
def index():
    if request.method == 'POST':
        ...
        if request.form.get('public', None) == 'New Public Codepad':
            codepadurl =             uniquelink = ''.join(random.choice(string.ascii_letters+string.digits) for i in range(9))
            return redirect(url_for('publicpad', padid=codepadurl))

        if request.form.get('docpad', None) == 'New Document Pad':
            docpadurl =             uniquelink = ''.join(random.choice(string.ascii_letters+string.digits) for i in range(8))
